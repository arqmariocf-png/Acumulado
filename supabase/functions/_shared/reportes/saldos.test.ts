...
import { construirReporteSaldosDia, etiquetaCuenta, type FilaSaldoCuenta } from "./saldos.ts";
...
test("construirReporteSaldosDia: agrupa por empresa y calcula subtotal + total", () => {
  ...
  assert.deepEqual(reporte.grupos[0].subtotal, { saldoInicial: 300, entradas: 50, salidas: 50, saldoFinal: 300 });
  assert.equal(reporte.grupos[1].empresaNombre, "Ergodinova");
  assert.deepEqual(reporte.grupos[1].subtotal, { saldoInicial: 500, entradas: 10, salidas: 0, saldoFinal: 510 });

  assert.deepEqual(reporte.total, { saldoInicial: 800, entradas: 60, salidas: 50, saldoFinal: 810 });
});
...
test("construirReporteSaldosDia: lista vacía produce cero grupos y totales en cero", () => {
  ...
  assert.deepEqual(reporte.total, { saldoInicial: 0, entradas: 0, salidas: 0, saldoFinal: 0 });
});
